...
!function(e,t){"use strict";var d={versions:[{nb:"2.0.0"}]};if(typeof t.fps_limit=="undefined")t.fps_limit=!1;var n=t;n.utils={hexToRgb:function(e){var t=/^#?([a-f\d])([a-f\d])([a-f\d])$/i;e=e.replace(t,function(e,t,n,i){return t+t+n+n+i+i});var n=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(e);return n?{r:parseInt(n[1],16),g:parseInt(n[2],16),b:parseInt(n[3],16)}:null},clamp:function(e,t,n){return Math.min(Math.max(e,t),n)},isInArray:function(e,t){return t.indexOf(e)>-1}}}(this,window);
...
function hexToRgb(e){var t=/^#?([a-f\d])([a-f\d])([a-f\d])$/i;e=e.replace(t,function(e,t,n,i){return t+t+n+n+i+i});var n=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(e);return n?{r:parseInt(n[1],16),g:parseInt(n[2],16),b:parseInt(n[3],16)}:null}
...
var existingCanvas=document.getElementById(tag_id).getElementsByClassName("particles-js-canvas-el");
if(existingCanvas.length>0){while(existingCanvas.length>0){document.getElementById(tag_id).removeChild(existingCanvas[0])}}
var canvas=document.createElement("canvas");
canvas.className="particles-js-canvas-el";
canvas.style.width="100%";
canvas.style.height="100%";
var node=document.getElementById(tag_id).appendChild(canvas);
if(node!==null){pJSDom.push(new pJS(tag_id,params))}
};
...
if(typeof(color.value.r)!="undefined"&&typeof(color.value.g)!="undefined"&&typeof(color.value.b)!="undefined"){this.color.rgb={r:color.value.r,g:color.value.g,b:color.value.b}}
if(typeof(color.value.h)!="undefined"&&typeof(color.value.s)!="undefined"&&typeof(color.value.l)!="undefined"){this.color.hsl={h:color.value.h,s:color.value.s,l:color.value.l}}
}
...
case"top-right":velbase={x:.5,y:-.5};break;
case"right":velbase={x:1,y:-0};break;
case"bottom-right":velbase={x:.5,y:.5};break;
case"bottom":velbase={x:0,y:1};break;
case"bottom-left":velbase={x:-.5,y:1};break;
case"left":velbase={x:-1,y:0};break;
case"top-left":velbase={x:-.5,y:-.5};break;
...
case"polygon":pJS.fn.vendors.drawShape(pJS.canvas.ctx,p.x-radius/(pJS.particles.shape.polygon.nb_sides/3.5),p.y-radius/.76,radius*2.66/(pJS.particles.shape.polygon.nb_sides/3),pJS.particles.shape.polygon.nb_sides,1);break;
case"star":pJS.fn.vendors.drawShape(pJS.canvas.ctx,p.x-radius*2/(pJS.particles.shape.polygon.nb_sides/4),p.y-radius/1.52,radius*2*2.66/(pJS.particles.shape.polygon.nb_sides/3),pJS.particles.shape.polygon.nb_sides,2);break;
case"image":
function draw(){pJS.canvas.ctx.drawImage(img_obj,p.x-radius,p.y-radius,radius*2,radius*2/p.img.ratio)}
...
var out_mode=pJS.particles.move.out_mode;
if(out_mode==="bounce"){
var new_pos={x_left:p.radius,x_right:pJS.canvas.w,y_top:p.radius,y_bottom:pJS.canvas.h}
}else{
var new_pos={x_left:-p.radius,x_right:pJS.canvas.w+p.radius,y_top:-p.radius,y_bottom:pJS.canvas.h+p.radius}
}
...
if(dist_p>=dist){p1.vx=-p1.vx;p1.vy=-p1.vy;p2.vx=-p2.vx;p2.vy=-p2.vy}
};
...
if(pJS.interactivity.modes.bubble.size>pJS.particles.size.value){var size=p.radius+pJS.interactivity.modes.bubble.size*ratio;if(size>=0){p.radius_bubble=size}}
else{var dsize=p.radius-pJS.interactivity.modes.bubble.size,size=p.radius-dsize*ratio;if(size>0){p.radius_bubble=size}else{p.radius_bubble=0}}
}
if(pJS.interactivity.modes.bubble.opacity!=pJS.particles.opacity.value){
if(pJS.interactivity.modes.bubble.opacity>pJS.particles.opacity.value){var opacity=pJS.interactivity.modes.bubble.opacity*ratio;if(opacity>p.opacity&&opacity<=pJS.interactivity.modes.bubble.opacity){p.opacity_bubble=opacity}}
else{var opacity=p.opacity-(pJS.particles.opacity.value-pJS.interactivity.modes.bubble.opacity)*ratio;if(opacity<p.opacity&&opacity>=pJS.interactivity.modes.bubble.opacity){p.opacity_bubble=opacity}}
}
}
}else{init()}
...
var repulseRadius=Math.pow(pJS.interactivity.modes.repulse.distance/6,3),dx=pJS.interactivity.mouse.click_pos_x-p.x,dy=pJS.interactivity.mouse.click_pos_y-p.y,d=dx*dx+dy*dy,force=-repulseRadius/d*1;
if(d<=repulseRadius){
...
for(var i=0;i<pJS.particles.array.length;i++){
var p2=pJS.particles.array[i],dx=p1.x-p2.x,dy=p1.y-p2.y,dist=Math.sqrt(dx*dx+dy*dy);
...
var svgXml=pJS.tmp.source_svg,coloredSvg=svgXml.replace(/#([0-9A-F]{3,6})/gi,function(match,p1,offset,string){
if(p.color.rgb){return"rgba("+p.color.rgb.r+","+p.color.rgb.g+","+p.color.rgb.b+","+p.opacity+")"}
else{return"hsla("+p.color.hsl.h+","+p.color.hsl.s+"%,"+p.color.hsl.l+"%,"+p.opacity+")"}
});
var svg=new Blob([coloredSvg],{type:"image/svg+xml;charset=utf-8"}),DOMURL=window.URL||window.webkitURL||window,url=DOMURL.createObjectURL(svg),img=new Image;
...
var sideCount=sideCountNumerator*sideCountDenominator,decimalSides=sideCountNumerator/sideCountDenominator,interiorAngleDegrees=(180*(decimalSides-2))/decimalSides,interiorAngle=Math.PI-Math.PI*interiorAngleDegrees/180;
